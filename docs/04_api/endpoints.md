...
> 💡 **작성 가이드**: 리소스별로 엔드포인트를 문서화합니다. **기계가 읽는 단일 소스는 OpenAPI 스펙**이며(→ [api_guidelines.md OpenAPI 정책](./api_guidelines.md#openapi-단일-소스-정책)), 이 문서는 그 스펙을 사람이 빠르게 읽기 위한 동반 문서입니다. 스키마가 바뀌면 OpenAPI를 먼저 고치고 이 문서를 맞춥니다(수기 중복 금지).
...
- **Base URL / 버저닝 / 인증 / 공통 헤더 / 에러 모델**: [api_guidelines.md](./api_guidelines.md) 인증·공통 헤더·에러 응답 모델을 따른다.
...
- **멱등성**: 생성/결제 등 부작용 있는 POST는 `Idempotency-Key` 헤더 필수(→ 공통 헤더).
...
- Response: `data[]` + `meta.next_cursor` (공통 규약의 페이지네이션).

> 다른 리소스는 위 양식을 복제해 절을 복제해 추가한다.
