...
> **역할-모델 분리 실현 방식 (실측 확정, 2026-06-16)**: hermes 내부 서브에이전트는 메인 모델을 상속하므로 hermes 단독으로는 Generator≠Verifier가 안 된다. → **ralph 외부 래퍼가 역할마다 `hermes -z "<프롬프트>" -m <모델> --provider openrouter`로 호출**해 LLM 레벨 분리를 보장한다. 모델은 모두 OpenRouter 경유(예: Generator=`openai/gpt-5.3-codex`, Verifier=`anthropic/claude-opus-4-8`). 구현·근거: [PoC ralph_loop.sh / EXPERIMENT](../../poc/hermes-agent/EXPERIMENT_role_model_separation.md).
...
| 실행기(runner)·비용 | 구독 CLI(codex/claude/gemini) = 추가 과금 0 | hermes(API) = 토큰 과금 |
