...
- resilience 패턴: api_guidelines 8.7(재시도/타임아웃/서킷브레이커/멱등성/벌크헤드). 충돌 회피로 endpoints를 8.8~8.10으로 이동 → 섹션 8이 8.1~8.10 연속.

## 남은 작업 (선택)
- **[정합] 양방향 상호참조 보완, 섹션번호 근본 비번호화(글로벌 하드코딩 번호 → 파일-로컬/무번호)**.
- 콘텐츠 심화(필수 항목)는 일단 완료. 이후는 실제 프로젝트에 템플릿을 채우는 단계.
