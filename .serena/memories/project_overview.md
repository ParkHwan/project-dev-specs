...
- **구독 모드 실증(2026-06-16 PASS)**: `ralph_loop.sh tasks.smoke.sub.json`(codex 생성/claude 검증)을 호스트에서 실행 → success, 추가 API 과금 0. 확인: 구독 CLI는 계정 기본 모델 사용(codex=gpt-5.5, tasks.json model은 의도 기록용·`-m` 무시). codex exec 기본 read-only라 생성 역할은 `--sandbox workspace-write` 필요(반영함). 그 1회는 calc.py 잔존본이라 통과 → 신규 생성은 calc.py 삭제 후 재검증 권장.
